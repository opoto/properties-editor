...
function decryptPValue(ciphered) {
  return ciphered.substring(4, ciphered.length - 1);
}
function encryptPValue(cleartxt) {
  return "ENC(" + cleartxt + ")";
}
...
function addProperty(name, value, desc) {
  var encbut, inputclass;
  if (value && value.startsWith("ENC(") && value.trim().endsWith(")")) {
    // value is encrypted
    // 1. decrypt it
    // TODO
    value = decryptPValue(value);
    // 2. mark as encrypted
    encbut = "<i class='icon encrypted' encrypted='true'></i>";
    inputclass = "class='encrypted-val'";
  } else {
    // not encrypted
    encbut = "<i class='icon cleartext' encrypted='false'></i>";
    inputclass = "";
  }
  ...
  $(addedRow).find(".tdenc i").click(function(event) {
    var encstatus = $(event.target),
      input = encstatus.parents("tr").find("input");

    if (encstatus.attr("encrypted") === "true") {
      encstatus.attr("encrypted", "false");
      encstatus.removeClass("encrypted");
      encstatus.addClass("cleartext");
      input.removeClass("encrypted-val");
    } else {
      encstatus.attr("encrypted", "true");
      encstatus.addClass("encrypted");
      encstatus.removeClass("cleartext");
...
function exportProperties() {
  ...
  $("#tprops tr").each(function (idx) {
    var isIncluded = $(this).find(".tdincl input[type=checkbox]").is(":checked");
    if (isIncluded) {
      var name = $(this).find("td.tdname").text().trim();
      var value = $(this).find(".tdvalue input[type=text]").val().trimLeft();
      if (config.vtrimr) {
        value = value.trimRight();
      }
      var isEncrypted = $(this).find(".tdenc i").attr("encrypted") === "true";
      if (isEncrypted) {
        value = encryptPValue(value);
      } else {
...
$("#save").click(function () {
  saveConfig();
  var properties = exportProperties();
  if (!properties) {
    return;
  }
  if (!config.debug) {
...
config.debug = localStorage.getItem("propedit.debug");
